...
| `/api/referrals/lookup/{code}` | GET | Hospital staff enter a referral code, get the record back. Unauthenticated by deliberate precedent (hospital staff has no login), same as `/api/screen` and `GET /api/workers/{phone}`. |
| `/api/referrals/lookup/{code}/confirm-attendance` | POST | Closes the loop: marks the referral attended. This is what makes referral completion rate (the headline KPI) measurable end to end. |
...
Last rewritten 10 August; touched again 14–15 August for the master doc v6.0 changes below — Teach Mode is cut (not built). The referral-code backend landed 15 August; the mobile-side card display and the USSD web simulator are still ahead.
...
2. **Smart referral, proven twice** (mobile) — the Referral Card, then Home's live "Refer Now" worklist showing the same referral with a colour-coded status pill and a tap-to-call phone number. **Backend live as of 15 August**: every ORANGE/RED referral now carries a `referral_code` (`SG-4K7Q` style), included in the miner's SMS and the hospital pre-alert, with `GET /api/referrals/lookup/{code}` and its confirm-attendance companion route live and unauthenticated — this is the master doc's Section 6 shot list, "a referral created, with facility, deadline and code." **Still ahead**: the mobile card actually displaying the code, and the hospital-side lookup page itself (Takudzwa's, against the now-finalized `docs/API_CONTRACT.md` contract).
3. **USSD reach** — **live as of 15 August**: our own USSD web simulator (`http://<host>/ussd-simulator`, master doc v6.0 Section 16.1) — a keypad/green-screen page hitting the real `/api/ussd` endpoint, framed on stage as "our simulator, their SMS" (the shortcode application is a regulatory process on the operator's timeline, not ours to control). The CLI simulator and Africa's Talking's own sandbox web simulator remain available as fallbacks.
