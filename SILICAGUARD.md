...
Kwekwe District Hospital records roughly one silicosis death a week — 50 to 60 a year in one district. Silicosis is incurable; early detection and removal from dust exposure are the only effective interventions. Zimbabwe has 500,000–1.5 million artisanal and small-scale gold miners with 19% silicosis prevalence, 6.8% TB, 18% HIV, and zero digital occupational screening infrastructure. The USAID-funded programme that used to screen this population (Kunda Nqob'iTB, 10,668 miners screened 2020–2023) lost its funding in 2024 and nothing replaced it.
...
4. **Enterprise Occupational Health** — bulk workforce onboarding, scheduled campaigns, aggregate-only risk dashboards and statutory compliance export for formal mining employers and insurers — the segment that makes the platform financially sustainable.
...
| Clinical and enterprise web dashboards | Worker mode in the app | Feature flag, switched on later for formal-sector employees |
...
| Dashboard | React + Vite + Recharts + Leaflet | Clinical and enterprise role-based views |
...
**Permanently removed, do not reintroduce**: Flutter/Dart, chest X-ray AI, WhatsApp as a channel, voice/IVR, the four-week education message sequence. See Section 12 for why.
...
|---|---|---|
| `workers` (currently `miners`) | `id, phone (unique, persistent identity), name, employer_id, site, job_role, created_at` | The worker register. Phone number is the identity across all screenings. |
...
| `employers` | `id, name, contact, subscription_tier, created_at` | Enterprise accounts. |
| `campaigns` | `id, employer_id, site, job_roles, start_date, end_date, target_count` | Scheduled screening campaigns. |
...
**Explicitly removed, not part of this schema**: `xray_results` table, `whatsapp_messages` table, `referrals.xray_completed` column, any `XRAY_UPLOADED` referral status.
...
| `/api/enterprise/workforce` | POST | Bulk CSV workforce upload |
| `/api/enterprise/aggregate` | GET | Aggregate-only employer queries. Must never return individual clinical records. |
| `/api/auth/login` | POST | JWT authentication with role claim: `practitioner`, `clinical`, `employer` |
...
**Not AI**: the mobile app UI, the dashboards, SMS dispatch, Teach Mode cards, the USSD decision tree, the outreach planner and the compliance export. Never describe these as AI.
...
5. **Dashboards + Outreach Planner** (web) — referral queue with one-tap close, outreach planner scheduling a visit, then the employer aggregate view explicitly showing individual clinical records are not exposed.
