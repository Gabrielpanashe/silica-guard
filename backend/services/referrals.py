...
def create_referral_and_notify(
    ...
    schema already had a place for, then calls the (currently stubbed)
    notification functions. pre_alert_sent stays 0 until real SMS/WhatsApp
    sending is wired in — it should reflect an actual message going out, not
    just a logged intent."""
    ...
    notifications.send_hospital_prealert(
        miner_name,
        phone_number,
        mine_site,
        risk_level,
