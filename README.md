...
Mobile (`mobile/`) and dashboard (`dashboard/`) apps are not yet scaffolded in this repo — see `CLAUDE.md` for ownership.

## Repo layout

- `backend/` — FastAPI backend: API routes, AI risk engine, USSD/SMS integration, database.
- `docs/` — the API contract shared between backend and both frontends.
- `mobile/` — React Native (Expo) practitioner app (not yet started).
- `dashboard/` — React web dashboard (not yet started).
