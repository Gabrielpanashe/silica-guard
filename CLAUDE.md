...
Last updated: 5 August 2026 (sprint Day 5 — counting forward from 1 August per the rule above; this section had gone stale by three days). `be/phase-a-schema-migration` **is merged to `main`** (PR #1, 2 August) — it is not a pending branch. Collaborator has a `feat/mobile-frontend` branch, based on that merged main, with Home/Intake/Question/Result/Referral screens already in progress.

**Phase A (schema migration), merged:**
...
- All 32 tests pass; `docs/API_CONTRACT.md` and `backend/scripts/seed_demo_data.py` updated to match.
...
**Scope pivot (5 August 2026, branch `docs/artisanal-only-pivot`, unmerged):** Dr Bopoto's feedback — formal mining companies already run robust pneumoconiosis programmes, so SilicaGuard should target artisanal miners only. Removed the Enterprise Occupational Health pillar entirely: the `employers`/`campaigns` tables (dropped from schema, not left dead), their seed rows, the target `employer` role and its privacy-boundary rule, and both `/api/enterprise/*` target routes. `outreach_visits` and `facilities` stay — they're not enterprise-specific. Full detail and the "who pays" rewrite are in `SILICAGUARD.md` Section 13. **The `.docx` master reference document has not been manually updated yet** — still describes the enterprise pillar; see the handoff checklist from this session for exact sections.

**Still not started**: hard-coded safety overrides living in Python outside the model call (currently still only in the Gemini prompt text — a non-negotiable-rule violation), Longitudinal Deterioration Detection logic (the link exists, the comparison doesn't), Smart Referral Router (facility matching, reminder cascade, escalation task — only the initial deadline is set), personalised advice generation, Teach Mode, Outreach Planner endpoints.
