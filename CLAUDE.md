...
- **Outreach Planner + SMS audit trail** (branch `be/outreach-planner-and-notifications`, 6 August, today):
  ...
  - **Render deployment is next** (repo-side prep + the persistent-storage decision — Render free tier has no persistent disk — deliberately not resolved yet, revisit when actually starting that piece).
