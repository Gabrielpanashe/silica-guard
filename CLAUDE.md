...
- **Mines dropdown + live VHW dashboard numbers** (branch `be/shona-mines-dashboard-live-testing`, 7 August, today, same session, not yet pushed):
  ...
  - **Not yet pushed** — sitting on `be/shona-mines-dashboard-live-testing` locally, held per explicit instruction to land everything as one branch/PR.
...
**Still not started**: Teach Mode, Clinical Lead sign-off on all draft copy (advice-line templates, `explanation_shona` templates, reminder/escalation/outreach/screening-result SMS text, the ORANGE Shona/English copy, the narrative prompt), auth role rename (`hospital`/`cimas` → target `practitioner`/`clinical` — deliberately declined for now, no remaining case in this product needs role differentiation post-pivot), phone-number hashing decision above needs the team's sign-off. USSD self-screening has never been tested over Africa's Talking's actual network (only the local `scripts/ussd_simulator.py`, which bypasses their infrastructure entirely) — no production shortcode was ever provisioned, and one realistically won't be before freeze; recommend testing via Africa's Talking's own sandbox web simulator instead (register the test number as a sandbox "Simulator Number" first) and framing the demo honestly as simulated-over-their-sandbox, not live-carrier. **Mobile integration**: unlike an earlier session's assessment, the mobile app (`feat/mobile-frontend` branch) genuinely does integrate with the real backend now — confirmed by a live end-to-end test today (see above) — but the pushed branch itself still has the stale `/api/miners`/`mine_site` bug described above, and three collaborator-owned bugs (nav crash, decorative referral card, repeat-screening offline fallback) have written-up-but-unapplied patches waiting in this session's scratchpad, flagged to the collaborator.
